...
namespace Meebey.SmartIrc4net
{
    ...
    public class IrcConnection
    {
        ...
        private Encoding         _ReadEncoding = Encoding.Default;
        ...
        /// <summary>
        /// Encoding which is used for reading to the socket
        ...
        /// <summary>
        /// causes the attampted detection of UTF formatted strings
        /// Default: false
        /// </summary>
        public bool DetectUTF
        {
            get
            {
                return _detectUTF;
...
#endif
                _Reader = new StreamReader(stream, _ReadEncoding);
                _Writer = new StreamWriter(stream, getWriteEncoding());

                if (getWriteEncoding().GetPreamble().Length > 0)
                {
        ...
        private Encoding getWriteEncoding()
        {
        ...
        private class ReadThread
        {
            ...
            {
                if (!_Connection._detectUTF)
                    return _Connection._Reader.ReadLine(); //_Connection.getWriteEncoding().GetString(Encoding.Convert(_Connection._ReadEncoding, _Connection.getWriteEncoding(), _Connection._ReadEncoding.GetBytes(_Connection._Reader.ReadLine())));

                else
                {
                    List<byte> lineBytes = new List<byte>();
                    Stream stream = _Connection._TcpClient.GetStream();
                    byte b;
                    int tempInt;
                    ...
                    bool isUTF = false;
                    byte[] finalBytes = lineBytes.ToArray();
                    ...
                    if (isUTF)
                        ...
                        return _Connection._ReadEncoding.GetString(finalBytes); //_Connection.getWriteEncoding().GetString(Encoding.Convert(_Connection._ReadEncoding, _Connection.getWriteEncoding(), finalBytes));

                }
            }

            private String decodeUTF8(byte[] data, out bool isUTF)
            {
                byte a, b, c;
